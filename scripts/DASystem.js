    ...
    if (itemData.armor.RealDL > 0 && itemData.armor.DL[tipoDano] > 0) {
        ...
        if (!options?.repairLvl) {
            ...
            if ((itemData.armor.DL.bldg < itemData.armor.RealDL) &&
                ...
                (itemData.armor.DL.pierc < itemData.armor.RealDL)) itemData.armor.RealDL = Math.max(0, itemData.armor.RealDL - 1);
            ...
            if ((itemData.armor.DL.bldg < itemData.armor.RealDL) &&
                (itemData.armor.DL.slsh < itemData.armor.RealDL) &&
                (itemData.armor.DL.pierc < itemData.armor.RealDL)) itemData.armor.RealDL = Math.max(0, itemData.armor.RealDL - options.repairLvl);
        }
...
export const computaREST = function (item, owner, amountRecovered) {
    ...
    if (itemData.armor.RealDL > 0) {
        ...
        if (itemData.armor.DL[tipoDanoMax] - amountRecovered < 0)
            ...
            itemData.armor.DL[tipoDanoMax] = Math.max(0, itemData.armor.DL[tipoDanoMax] - amountRecovered);
        ...
        if ((itemData.armor.DL.bldg < itemData.armor.RealDL) &&
            (itemData.armor.DL.slsh < itemData.armor.RealDL) &&
            (itemData.armor.DL.pierc < itemData.armor.RealDL)) itemData.armor.RealDL = Math.max(0, itemData.armor.RealDL - amountRecovered);
...
export const rollRepair = async function (item, owner, options = {}) {
    if (!options.smithRepairChk) {
        ...
        const rollDC = adControl.DC_REPAIR[item.system.rarity.toLowerCase()] ?? 10;
        const roll = await owner.rollToolCheck(config, options);
        if (!roll || roll.lenght === 0) return false;
        return (roll[0]._total >= rollDC);
    } else return true;
}
...
export const computeEquipArmorShield = async function (actor, item, action) {
    ...
    const flags = {
        armor: actor.getFlag("ldnd5e", "armorEffect"),
        shield: actor.getFlag("ldnd5e", "shieldEffect")
    };

    const effects = {
        armor: actor.effects.get(flags.armor.effectID),
